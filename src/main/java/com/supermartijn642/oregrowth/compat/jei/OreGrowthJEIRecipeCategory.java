...
public class OreGrowthJEIRecipeCategory implements IRecipeCategory<OreGrowthRecipe> {
    ...
    public void setRecipe(IRecipeLayoutBuilder layoutBuilder, OreGrowthRecipe recipe, IFocusGroup focusGroup){
        ...
        layoutBuilder.addSlot(RecipeIngredientRole.CATALYST, columns == 1 ? 11 : 2, 24)
            .setSlotName("base")
            .addItemStacks(recipe.bases().stream().map(Block::asItem).map(Item::getDefaultInstance).toList())
            .setCustomRenderer(VanillaTypes.ITEM_STACK, new IIngredientRenderer<>() {
                ...
                public List<Component> getTooltip(ItemStack stack, TooltipFlag flag){
                    return OreGrowthJEIRecipeCategory.this.itemStackRenderer.getTooltip(stack, flag);
                }

                @Override
                public Font getFontRenderer(Minecraft minecraft, ItemStack ingredient){
                    return OreGrowthJEIRecipeCategory.this.itemStackRenderer.getFontRenderer(minecraft, ingredient);
                }
    ...
    public void draw(OreGrowthRecipe recipe, IRecipeSlotsView slotsView, PoseStack poseStack, double mouseX, double mouseY){
        ...
        ClientUtils.getItemRenderer().renderAndDecorateFakeItem(Items.DIAMOND_PICKAXE.getDefaultInstance(), 43, 18);
        ...
        Block base = slotsView.findSlotByName("base")
            ...
            .orElse(null);
        if(base != null)
            renderModel(poseStack, base.defaultBlockState(), 9, 31, 0, EmptyModelData.INSTANCE);
        ...
        if(base != null){
            ...
            IModelData modelData = new ModelDataMap.Builder().withInitial(OreGrowthBlockBakedModel.BASE_BLOCK_PROPERTY, base).build();
    ...
    private static void renderModel(PoseStack poseStack, BlockState state, int x, int y, int offset, IModelData modelData){
