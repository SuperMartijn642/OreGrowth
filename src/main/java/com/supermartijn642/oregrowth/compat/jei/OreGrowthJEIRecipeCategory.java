...
public class OreGrowthJEIRecipeCategory implements IRecipeCategory<OreGrowthRecipe> {
    ...
    public void draw(OreGrowthRecipe recipe, IRecipeSlotsView slotsView, PoseStack poseStack, double mouseX, double mouseY){
        ...
        Block base = slotsView.findSlotByName("base")
            ...
            .orElse(null);
        if(base != null)
            renderModel(poseStack, base.defaultBlockState(), 9, 31, 0);
        ...
        if(base != null){
            ...
            if(model instanceof OreGrowthBlockBakedModel)
                ((OreGrowthBlockBakedModel)model).withContext(base, () -> renderModel(poseStack, state, 9, 15, 10));
            else
                renderModel(poseStack, state, 9, 15, 10);
        }
