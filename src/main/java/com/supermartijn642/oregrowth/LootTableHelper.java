...
public class LootTableHelper {
    ...
    private static LootEntryConditions formatCondition(LootItemCondition condition){
        if(condition instanceof PredicateManager.CompositePredicate){
            List<LootEntryConditions> subs = Arrays.stream(((PredicateManager.CompositePredicate)condition).terms).map(LootTableHelper::formatCondition).filter(Objects::nonNull).toList();
            if(subs.isEmpty())
                return null;
            if(subs.size() == 1)
                return subs.get(0);
        ...
        if(condition instanceof AlternativeLootItemCondition){
            List<LootEntryConditions> subs = Arrays.stream(((AlternativeLootItemCondition)condition).terms).map(LootTableHelper::formatCondition).filter(Objects::nonNull).toList();
            if(subs.isEmpty())
