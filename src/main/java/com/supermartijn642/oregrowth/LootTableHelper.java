...
public class LootTableHelper {
    ...
    private static MutableComponent formatItemPredicate(ItemPredicate predicate){
        ...
        List<Enchantment> actualEnchants = predicate.enchantments().stream().map(EnchantmentPredicate::enchantment).filter(Optional::isPresent).map(Optional::get).map(Holder::value).toList();
        if(actualEnchants.size() == 1)
            enchantments = TextComponents.translation(actualEnchants.get(0).getDescriptionId()).color(ChatFormatting.GOLD).get();
        else if(actualEnchants.size() == 2){
            Component enchant1 = TextComponents.translation(actualEnchants.get(0).getDescriptionId()).color(ChatFormatting.GOLD).get();
            Component enchant2 = TextComponents.translation(actualEnchants.get(1).getDescriptionId()).color(ChatFormatting.GOLD).get();
            enchantments = TextComponents.translation("oregrowth.jei_category.conditions.match_tool.two_items", enchant1, enchant2).get();
        }else if(actualEnchants.size() > 2){
            TextComponents.TextComponentBuilder builder = TextComponents.translation(actualEnchants.get(0).getDescriptionId()).color(ChatFormatting.GOLD);
            for(int i = 1; i < actualEnchants.size() - 1; i++)
                builder = builder.string(", ").append(TextComponents.translation(actualEnchants.get(i).getDescriptionId()).color(ChatFormatting.GOLD).get());
            Component lastEnchant = TextComponents.translation(actualEnchants.get(actualEnchants.size() - 1).getDescriptionId()).color(ChatFormatting.GOLD).get();
        ...
        if(predicate.tag().isPresent()){
            if(enchantments == null)
                return TextComponents.translation("oregrowth.jei_category.conditions.match_tool.tag", predicate.tag().get().location()).get();
            else
                return TextComponents.translation("oregrowth.jei_category.conditions.match_tool.tag", predicate.tag().get().location()).translation("oregrowth.jei_category.conditions.match_tool.enchanted", enchantments).get();
        }
